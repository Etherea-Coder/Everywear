...
class _PhotoCaptureSectionState extends State<PhotoCaptureSection> {
  ...
  Future<void> _takePhoto() async {
    try {
      ...
      if (photo != null) {
        ...
        // Run AI analysis on the captured photo
        await _analyzePhoto(photo.path);
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _pickFromGallery() async {
    try {
      ...
      if (image != null) {
        final updatedPhotos = List<String>.from(widget.photos)..add(image.path);
        widget.onPhotosChanged(updatedPhotos);

        // Run AI analysis on the selected photo
